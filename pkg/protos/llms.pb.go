...
type AnalyzeArticleWithProviderModelRequest struct {
	...
	Glossary       *bool                  `protobuf:"varint,10,opt,name=glossary,proto3,oneof" json:"glossary,omitempty"`                            // When set, overrides the server's glossary config (per-article term extraction + cached definitions)
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}
...
func (x *AnalyzeArticleWithProviderModelRequest) GetProvider() string {
	if x != nil {
		return x.Provider
	}
	return ""

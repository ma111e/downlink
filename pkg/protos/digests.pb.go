...
type GenerateDigestRequest struct {
	...
	Glossary               *bool                  `protobuf:"varint,17,opt,name=glossary,proto3,oneof" json:"glossary,omitempty"`                                                         // When set, overrides the server's glossary config for this run (per-article term extraction + cached definitions)
	unknownFields          protoimpl.UnknownFields
	sizeCache              protoimpl.SizeCache
}
...
func (x *GenerateDigestRequest) GetReanalyzeOnModelChange() bool {
	if x != nil {
		return x.ReanalyzeOnModelChange
	}
	return false
}

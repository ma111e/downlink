...
// Lease is a handle to a live credential for a single LLM call. Call one of the
// Mark* methods when done to update the credential's health state.
type Lease struct {
	CredID      string
	AccessToken string
	Headers     http.Header
...
// expiringWithin reports whether the credential's access token is absent or due
// to expire within skew. A nil ExpiresAt is treated as expiring so the pool
// refreshes rather than sending a token of unknown age.
func expiringWithin(c *models.CodexCredential, skew time.Duration) bool {
	if c.ExpiresAt == nil {
		return true
	}
...
func (p *Pool) Acquire(ctx context.Context) (*Lease, error) {
	...
	for i := range p.creds {
		...
		if !expiringWithin(c, refreshSkew) {
			lease := &Lease{
				CredID:      c.Id,
				AccessToken: c.AccessToken,
				Headers:     ClaudeHeaders(c.AccessToken),
				pool:        p,
			}
			p.mu.Unlock()
			return lease, nil

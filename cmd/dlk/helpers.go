...
// selectModelInteractive shows a huh picker over available models and returns the
// chosen provider type and model id. When providerFilter is empty it lists models
// from every enabled provider (each option labelled with its provider); when set
// it lists only that provider type's models. The provider type is returned
// alongside the model so the downstream call is unambiguous.
// Returns ("", "", nil) on cancel.
func selectModelInteractive(client *downlinkclient.DownlinkClient, providerFilter string) (providerType, modelName string, err error) {
	resp, err := client.GetAvailableModels()
	if err != nil {
		return "", "", fmt.Errorf("failed to get available models: %w", err)
	}
	if resp == nil || len(resp.Models) == 0 {
		return "", "", fmt.Errorf("no models available from any configured provider")
	}

	var matching []models.ModelInfo
	for _, m := range resp.Models {
		if providerFilter == "" || strings.EqualFold(m.ProviderType, providerFilter) {
			matching = append(matching, m)
		}
	}
	if len(matching) == 0 {
		if providerFilter != "" {
			return "", "", fmt.Errorf("no models available for provider %q", providerFilter)
		}
		return "", "", fmt.Errorf("no models available")
	}

	// Encode provider type + model id in the option value (unit separator).
	const sep = "\x1f"
	opts := make([]huh.Option[string], len(matching))
	for i, m := range matching {
		label := m.Name
		if m.DisplayName != "" && m.DisplayName != m.Name {
			label = fmt.Sprintf("%s (%s)", m.Name, m.DisplayName)
		}
		// Always show which provider the model belongs to when listing across all.
		if providerFilter == "" && m.ProviderType != "" {
			label = fmt.Sprintf("%s  ·  %s", label, m.ProviderType)
		}
		if m.Description != "" {
			label += "\n   " + m.Description
		}
		opts[i] = huh.NewOption(label, m.ProviderType+sep+m.Id)
	}

	title := "Select model"
	if providerFilter != "" {
		title = fmt.Sprintf("Select model for %s", providerFilter)
	}
	...
	parts := strings.SplitN(choice, sep, 2)
	if len(parts) != 2 {

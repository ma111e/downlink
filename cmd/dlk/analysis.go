...
Batch Analysis by Feed/Time:
  ...
  downlink-cli analysis run --dry-run --from 7d   # Preview matching articles`,
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			// Handle --select-model flag: list models (all providers when --provider
			// is omitted, otherwise filtered to that provider) and let the user pick.
			if runSelectModel {
				pt, mn, err := selectModelInteractive(client, providerType)
				if err != nil {
					fmt.Printf("%v\n", err)
					return
				}
			...
			if len(args) == 1 && runFrom == "" && runTo == "" && runBetween == "" && !runDryRun {
				...
				analyzeWithProfile := func(profile string) bool {
					...
					if profile != "" {
						...
						analysis, err = client.StreamAnalyzeArticleWithSelection(articleId, providerType, modelName, runKeyPointsOnly, onEvent)
					} else {
			...
			fmt.Printf("\nUse 'downlink-cli queue status' to monitor progress\n")
		},
	}

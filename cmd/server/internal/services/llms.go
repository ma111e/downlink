...
Return ONLY the JSON object below.`, countWord, strings.Join(levelLines, "\n")),
  ...
  - type: ONE of threat-actor, malware, tool, technique, vulnerability, protocol, concept, organization, product, location, person, other. Use "location" for any place — city, town, state, province, region, continent, or country. Use "person" for any named individual (a researcher, executive, official, or someone quoted). Classify these honestly even though they are not security terms; they are dropped afterwards.
...
Return one entry per distinct term the article uses, up to a maximum of 30; do not stop early to keep the list short. If the article genuinely uses no notable terms, return an empty array.
...
products — named products, platforms, software, hardware, frameworks, or technical systems the article is about: the concrete things a vendor or project actually builds, ships, sells, or maintains (e.g. globalprotect, fortios, exchange-server, kubernetes, log4j, activemq). Prefer the specific product name over its category. Use fortios not firewall, exchange-server not email-server. Open standards, protocols, and specifications — abstract rules defined by a standards body or RFC rather than an implementation owned and shipped by one vendor — are NOT products; leave them out of this axis even when the article centers on them. A concrete implementation is still a product, though: the standard a library like openssl implements is not a product, but openssl itself is.
